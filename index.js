...
function additem()
{
    ...
    if (it.value!=0)
    {
        ...
        c.textContent = document.getElementById('amount').value;
        tot.textContent ='$'+document.getElementById('amount').value*125;
...
function deletecart()
{
    document.getElementById('itemcart').textContent=0;
...
document.addEventListener("DOMContentLoaded", function() {  
    ...
    window.addEventListener('click', function(e) {
        if (mm.style.opacity='1' && e.target != mm && e.target != 'menu' && e.target.id!='ul' && e.target.id!='il' && e.target!=mh && e.target!=cl && e.target.classList!='ahref') {
                if (document.getElementById('menuhamb').style.display!='none')
                {
                    mm.style.left='-320px';
                    bd.classList.remove('op');
                }              
